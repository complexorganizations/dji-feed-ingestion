...
function check-rtsp-server-status() {
    ...
    if [ "$(ffprobe -v quiet -print_format json -show_streams ${RTSP_SERVER_ZERO} | wc -m)" ] >= 1000; then
        AWS_ACCESS_KEY_ID=KEY AWS_SECRET_ACCESS_KEY=KEY AWS_DEFAULT_REGION=us-east-1 ./kvs_gstreamer_sample dji-stream-0 ${RTSP_SERVER_ZERO}
    elif [ "$(ffprobe -v quiet -print_format json -show_streams ${RTSP_SERVER_ONE} | wc -m)" ] >= 1000; then
        AWS_ACCESS_KEY_ID=KEY AWS_SECRET_ACCESS_KEY=KEY AWS_DEFAULT_REGION=us-east-1 ./kvs_gstreamer_sample dji-stream-1 ${RTSP_SERVER_ONE}
    elif [ "$(ffprobe -v quiet -print_format json -show_streams ${RTSP_SERVER_TWO} | wc -m)" ] >= 1000; then
        AWS_ACCESS_KEY_ID=KEY AWS_SECRET_ACCESS_KEY=KEY AWS_DEFAULT_REGION=us-east-1 ./kvs_gstreamer_sample dji-stream-2 ${RTSP_SERVER_TWO}
    elif [ "$(ffprobe -v quiet -print_format json -show_streams ${RTSP_SERVER_THREE} | wc -m)" ] >= 1000; then
        AWS_ACCESS_KEY_ID=KEY AWS_SECRET_ACCESS_KEY=KEY AWS_DEFAULT_REGION=us-east-1 ./kvs_gstreamer_sample dji-stream-3 ${RTSP_SERVER_THREE}
    else
        exit
    fi
}
