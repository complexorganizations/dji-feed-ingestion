...
AMAZON_KINESIS_VIDEO_STREAMS_BIN_PATH="${AMAZON_KINESIS_VIDEO_STREAMS_PRODUCER_PATH}/bin"
...
function build-kensis-application() {
    if [ ! -d "${AMAZON_KINESIS_VIDEO_STREAMS_PRODUCER_PATH}" ]; then
        ...
        echo -e "export GST_PLUGIN_PATH=${AMAZON_KINESIS_VIDEO_STREAMS_PRODUCER_PATH}:$GST_PLUGIN_PATH\nexport PATH=${AMAZON_KINESIS_VIDEO_STREAMS_BIN_PATH}:$PATH\nexport LD_LIBRARY_PATH=${AMAZON_KINESIS_VIDEO_STREAMS_OPEN_SOURCE_LOCAL_LIB_PATH}:$LD_LIBRARY_PATH" >>~/.profile
        source ~/.profile
    fi
}
